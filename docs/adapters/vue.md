...
> 🧭 **手动模式**：属于 [两种使用模式](../getting-started/two-modes.md) 中的模式二（手动绑定）。`PiyingView` 是自动模式入口，而 `PiyingFieldTemplate` / `PiyingFieldControlBind` / `convertToField` 是手动模式的手动绑定工具。
...
详见 [Vue 强类型组件](vue-typed-component.md)。
...
> 🧭 **手动模式**：`convertToField` 是手动模式的核心入口，返回 `field` 后需用 `PiyingFieldControlBind` / `PiyingFieldTemplate` 手动绑定渲染位置。自动模式中 `PiyingView` 会在内部自动调用它，无需手动使用。详见 [两种使用模式](../getting-started/two-modes.md)。
...
- [Vue 强类型组件](vue-typed-component.md) — typedComponent 详解
- [框架差异](../getting-started/framework-differences.md) — Vue 的 Field Token / CVA 绑定
