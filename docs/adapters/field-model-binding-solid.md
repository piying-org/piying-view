...
> **与 React 的差异：** Solid 版本的 `use-*Model` 均返回 `createMemo` 结果，需调用 `()`；且 `compositionMode` 参数为函数 `() => boolean`。React 版本返回普通对象、`compositionMode` 为布尔值，详见 [React 版本](field-model-binding-react.md)。
...
- [Solid API](solid.md) — `@piying/view-solid` 完整 API
- [React 版本](field-model-binding-react.md) — React 的 `use-*Model` 签名
- [框架差异](framework-differences.md) — 各框架的 CVA 绑定与 Signal 转换工具对比
- [基础字段定义](../scenarios/basic-field.md) — setComponent / formConfig 用法
