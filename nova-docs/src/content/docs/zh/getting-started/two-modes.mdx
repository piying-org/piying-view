...
export class CustomFormComponent {
  // 注意：schema 与 options 都要以「取值函数」形式传入；injector 传 undefined，内部会自动获取
...
- 输入：`fieldTemplate`（必填，`PiResolvedViewFieldConfig`）、`path`（可选 `KeyPath`，不传则渲染整个根字段）
...
把上面两种方式合在一起：`k1` 用 `[formControl]` 绑定原生 input（输入不足 3 个字符可看到验证错误）；`k2` 用 `[fieldTemplate]` 插入，内部由 Schema 元数据自动渲染：
...
// schema 与 options 均以取值函数形式传入；injector 传 undefined，内部会自动获取
bind = convertToField(() => this.schema, undefined, () => this.options);
```
