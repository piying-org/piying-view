...
> 💡 **先分清两种模式**：本文展示的是**自动模式**（`<piying-view>` 组件 + Schema 全自动渲染）。还有一种**手动模式**（`convertToField` + `[formControl]`/`[fieldTemplate]` 手动绑定）。两种模式边界不同，详见 [两种使用模式](zh/getting-started/two-modes/)。
...
在 Angular 组件中，通过 `fieldGlobalConfig.types` 将类型名映射到具体组件（`InputComponent` 的编写方式见下文「如何编写字段组件」）：

```typescript
import { Component, signal } from '@angular/core';
import { PiyingView, PiyingViewGroup } from '@piying/view-angular';
import * as v from 'valibot';
import { InputComponent } from './input.component'; // string / number 通用的输入框组件
...
上面注册时引用的 `InputComponent`（即 `./input.component`），完整写法如下：
...
> **提示**：`setComponent`、`formConfig` 等 Actions 的用法详见 [基础字段定义](zh/scenarios/basic-field/)；[API: setComponent](zh/api/setcomponent/) — 包含字符串引用与直接传组件类的完整对比。

运行 `ng serve`，打开浏览器访问页面，你会看到一个包含 name、age、email 三个字段的表单。输入数据时，`model` signal 的值会同步更新；

## 下一步

- 了解 [两种使用模式](zh/getting-started/two-modes/) — 自动模式 vs 手动模式的边界划分
- 了解 [核心概念](zh/getting-started/core-concept/) — Schema 是如何转换为视图的
- 查看 [基础字段定义](zh/scenarios/basic-field/) — setComponent / formConfig 的详细用法
- 了解 [Options 配置](zh/getting-started/options-config/) — context / fieldGlobalConfig / builder 入门讲解
- 查看 [fieldGlobalConfig API](zh/api/global-config/) — types/wrappers 优先级体系完整参考
