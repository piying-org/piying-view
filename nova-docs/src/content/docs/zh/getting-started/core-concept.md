---
title: "核心概念：从 Schema 到 View"
---



> 💡 本文介绍的是两种使用模式**共享的底层解析链**。具体到「如何渲染」，分为自动模式（`<piying-view>` 全自动）与手动模式（`convertToField` + 指令手动绑定），边界见 [两种使用模式](zh/getting-started/two-modes/)。
...
- [fieldGlobalConfig API](zh/api/global-config/) — types/wrappers 优先级体系完整参考
- [基础字段定义](zh/scenarios/basic-field/) — 实战：使用 setComponent / formConfig
