...
| 验证方式     | 说明                            | 特点                               |
| ------------ | ------------------------------- | ---------------------------------- |
| Valibot 验证 | 在 Valibot 解析阶段进行，先执行 | 只能验证自身，无法以上下文值验证     |
| 控件自带验证 | 在 Valibot 验证成功后执行       | 可访问父级，根据其他字段值判断      |
...
- [API: FieldFormConfig](zh/api/field-config/) — validators / asyncValidators 配置详解
