...
> React 版本的 `use-*Model` 返回普通对象，直接 `{...model}` 展开即可。Solid 版本签名不同（返回 `createMemo` 计算信号），详见 [Solid 版本](zh/adapters/solid/field-model-binding/)。
...
- [Solid 版本](zh/adapters/solid/field-model-binding/) — Solid 的 `use-*Model` 签名差异
