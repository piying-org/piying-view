...
`@piying/view-solid` 专用。在 `typedFieldPipe` 的基础上多传一个「组件」，让 `inputs` / `outputs` 的 key 和值类型跟着这个 Solid 组件走。
...
## 懒加载：必须用 Solid 的 `lazy()`

Solid 适配层直接拿配置里的 `type` 渲染，运行时不解懒加载函数，所以必须过 `solid-js` 的 `lazy()`（模块要有 `default` 导出）：
...
`lazy()` 返回的是 props 已展开的函数组件，类型解析走首参分支，照常收窄。

`lazyMark()` 是通用标记，但只在**类型层**解开 `() => Promise<组件>`；Solid 运行时不消费它，单用渲染不出来。

## attributes / events 命名

- `attributes` 的标准名取自 Solid 自带的 `JSX.HTMLAttributes`，并**摘掉 `onXxx`**（那是 events 的地盘）。
...
> Solid 里 class 就是 `class`；`classList` 是 Solid 内置 prop，不在 attributes 的命名空间里。
...
- [Solid 包 API 参考](zh/adapters/solid/) — Token / CVA / createSignalConvert
- [字段模型绑定](zh/adapters/solid/field-model-binding/) — `use-*Model` 系列
