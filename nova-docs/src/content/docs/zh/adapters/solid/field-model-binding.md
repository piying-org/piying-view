...
> **与 React 的差异：** Solid 版本的 `use-*Model` 均返回 `createMemo` 结果，需调用 `()`；且 `compositionMode` 参数为函数 `() => boolean`。React 版本返回普通对象、`compositionMode` 为布尔值，详见 [React 版本](zh/adapters/react/field-model-binding/)。
...
- [React 版本](zh/adapters/react/field-model-binding/) — React 的 `use-*Model` 签名
