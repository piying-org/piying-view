...
export class CustomFormComponent {
  // note: both schema and options must be passed as getter functions; pass undefined as the injector — it is resolved automatically
  bind = convertToField(() => this.schema, undefined, () => this.options);
...
<LivePreview path="src/angular/components/two-modes-manual-demo.component.*">
...
// both schema and options are passed as getter functions; pass undefined as the injector — it is resolved automatically
bind = convertToField(() => this.schema, undefined, () => this.options);
```
...
- [AbstractControl](en/api/control-api/) — operating on controls through `field.form.control`
