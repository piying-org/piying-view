...
export const registerSchema = v.object({
  ...
  email: v.pipe(v.optional(v.email('Please enter a valid email address')), setComponent('input')),
...
| ---------- | ----------------------------------- | -------------------------- |
| Required validation | `formConfig({ required: true })` | + Valibot `v.minLength()`  |
| Custom validation | `formConfig({ validators: [...] })` | Password strength check, confirmation match |
| Conditional hiding | `hideWhen`                         | Toggling the public email setting |
| Conditional disabling | `disableWhen`                  | Disables the confirmation box until a password exists |
| Dynamic array | `v.array()` + FieldArray API         | Adding/removing tags        |
| Nested containers | `fieldGlobalConfig.types.object` | Group renders children recursively |
