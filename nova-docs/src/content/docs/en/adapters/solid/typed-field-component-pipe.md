---
title: "typedFieldComponentPipe (Solid)"
---

Solid only (`@piying/view-solid`). It adds one "component" argument on top of `typedFieldPipe`, so `inputs` / `outputs` keys and value types follow that Solid component.
...
## Lazy loading: must use Solid's `lazy()`

The Solid adapter renders the `type` from the config directly and never unwraps a lazy function at runtime, so it must go through `solid-js`'s `lazy()` (the module needs a `default` export):
...
const fieldGlobalConfig = { types: { emit: { type: solidLazyEmit } } };
...
A component returned by `lazy()` already has its props expanded, so type resolution just follows the first-parameter branch.

`lazyMark()` is the universal marker, but it only unwraps `() => Promise<Component>` **at the type level**; the Solid runtime does not consume it, so on its own it will not render.

## attributes / events naming

- Standard `attributes` names come from Solid's own `JSX.HTMLAttributes`, with **`onXxx` removed** (that is the events namespace's territory).
...
> In Solid the class attribute is `class`; `classList` is a Solid built-in prop and is not part of the attributes namespace.
...
- [Solid Package API](en/adapters/solid/) — tokens / CVA / createSignalConvert
- [Field Model Binding](en/adapters/solid/field-model-binding/) — the `use-*Model` family
