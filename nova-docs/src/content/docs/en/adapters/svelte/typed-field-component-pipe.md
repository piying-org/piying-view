...
Svelte only (`@piying/view-svelte`). It adds one "component" argument on top of `typedFieldPipe`, so `inputs` / `outputs` keys and value types follow that Svelte 5 component.
...
The Svelte adapter unwraps lazy imports at runtime; mark the function with `lazyMark()` and the type layer unwraps it into the real component too:
...
- Standard `attributes` names come from `svelte/elements`' `HTMLAttributes`, with **`onXxx` removed** (that is the events namespace's territory).
...
- A `Snippet` (`{#snippet}` / children) is callable as far as types go, but it is **content, not an event**, so it never appears among the `outputs` keys.
- Fields are obtained via `getContext(PI_VIEW_FIELD_TOKEN)` rather than dependency injection.

## Related

- [typedFieldPipe](en/api/typed-field-pipe/) — the generic, framework-agnostic version
- [Svelte Package API](en/adapters/svelte/) — getContext / signalToState / CVA
