...
Unlike `patchAsync`, which assigns **per key**, `mapAsync` receives the **whole attributes object** and transforms it functionally. It takes the shape `(field) => (value) => newValue`:
