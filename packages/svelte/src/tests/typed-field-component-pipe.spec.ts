...
import type {
	...
	SvelteAttributeName,
	SvelteStandardAttrName
} from '../lib/util/component-types';
import { createComponent } from './util/create-component.svelte';
import { delay } from './util/delay';
import TypedEmit from './component/typed-emit.svelte';
import TypedEmitMulti from './component/typed-emit-multi.svelte';
import TypedInputs from './component/typed-inputs.svelte';
...
import AnyInputs from './component/any-inputs.svelte';
...
const lazyEmit = () => import('./component/typed-emit.svelte').then((m) => m.default);
const lazyDefine = { types: { emit: { type: lazyMark(lazyEmit) } } };
...
describe('typedFieldComponentPipe - inputs 强类型', () => {
	...
	it('类型: props 与 emits 的拆分符合预期', () => {
		...
		const emits: Equal<keyof GetComponentEmits<typeof TypedEmit>, 'onChange' | 'onSubmit'> = true;

		expect([inputs, emits]).toEqual([true, true]);
	});

	it('类型: emit 处理器保留真实参数元组', () => {
		const change: Equal<GetComponentEmits<typeof TypedEmit>['onChange'], (value: number) => any> =
			true;
		const submit: Equal<GetComponentEmits<typeof TypedEmit>['onSubmit'], () => any> = true;
...
describe('typedFieldComponentPipe - inputs 真实值类型(非 any)', () => {
	...
	it('类型: 多参数 emit 回调保留完整参数元组', () => {
		...
		const range: Equal<Emits['onRange'], (start: number, end: string) => any> = true;
		const tag: Equal<Emits['onTag'], (index: number, tag: string, ok: boolean) => any> = true;
	...
	it('运行时: 强类型 input 真的渲染到组件上', async () => {
		...
		expect(instance.container.querySelector('.enabled')?.textContent).toBe('true');
		expect(instance.container.querySelector('.tags')?.textContent).toBe('x,y');
		expect(instance.container.querySelector('.meta')?.textContent).toBe('9:nine');
	});
	...
	it('运行时: 多参数 emit 的回调真的收到完整参数', async () => {
		...
		fireEvent.click(instance.container.querySelector('.btn-range') as HTMLElement);
		fireEvent.click(instance.container.querySelector('.btn-tag') as HTMLElement);
...
describe('typedFieldComponentPipe - outputs 强类型', () => {
	...
	it('类型: 零 output 组件跨字段监听, output 名不再塌成 never', () => {
		const ok = typedFieldComponentPipe(nfcPair, baseDefine, (d) => [
			d(['a'], 'empty', [
				d.outputChange((fn) => {
					fn([{ list: ['..', 'b'], output: 'onChange' }]).subscribe(({ listenFields }) => {
	...
	it('运行时: 零 output 组件跨字段监听, 照样收到对方 emit 的参数', async () => {
		...
		const merged = typedFieldComponentPipe(nfcPair, baseDefine, (d) => [
			d(['a'], 'empty', [
				d.outputChange((fn) => {
					fn([{ list: ['..', 'b'], output: 'onChange' }]).subscribe((stream) => {
		...
		fireEvent.click(instance.container.querySelector('.btn-change') as HTMLElement);
		await delay(10);
	...
	it('Svelte 版不暴露 models: 没有双向绑定通道', () => {
		const bad = typedFieldComponentPipe(numOnly, baseDefine, (d) => [
			d(['num'], 'emit', [
				// @ts-expect-error Svelte 运行时不消费 field.models
				d.models.patch({})
			])
		]);
...
describe('typedFieldComponentPipe - 运行时', () => {
	it('inputs.patch 真的下发到组件', async () => {
		...
		expect(instance.container.querySelector('.name')?.textContent).toBe('hello');
	...
	it('每条 entry 都自动下发 setComponent', async () => {
		...
		expect(instance.container.querySelector('.inputs-test')?.textContent).toContain(
			'auto-component'
		);
	});
...
describe('typedFieldComponentPipe - 懒加载组件', () => {
	...
	it('类型 + 运行时: lazyMark 配置下依旧收窄并渲染', async () => {
		...
		expect(bad).toBeTruthy();
		expect(instance.container.querySelector('.name')?.textContent).toBe('marked-name');
	});
	...
	it('运行时: 懒加载组件渲染并下发 input', async () => {
		...
		const { instance } = await createComponent(merged, undefined, {
			defaultConfig: { types: { emit: { type: lazyMark(lazyEmit) } } }
		});
		await delay(80);
