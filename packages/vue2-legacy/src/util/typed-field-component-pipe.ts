...
import { createDefineComponentEntry, typedFieldPipe, ɵtypedFieldActions } from '@piying/view-core';
import type {
 ...
 * 组件类型 -> 表。
 *
 * 这是 Vue 2 侧唯一需要写的「读组件」逻辑:
 * 把实例 `$props` 与 `emits` 声明打包成 `CompTables` 的形状,
 * 之后骨架(工厂 / entry / outputChange)全在 core 里复用一份。
 *
 * Vue 2 的 v-model 就是 prop + `update:xxx` 事件的约定, 运行时不消费 field.models,
 ...
 * 所以这里直接复用 core 的骨架。
 * Vue 2 侧额外摘掉 `models`(运行时不消费) 与 `attributes`(换成 HTML 名约束)。
 */
export type TypedComponentActionFactories<Cfg = unknown> = TypedComponentActionFactoriesOf<
  Cfg,
  Vue2AttributeName
>;

/** 可用的组件标识: 配置里注册的类型 key, 直接传组件, 或者直接传懒加载函数 */
export type ComponentKeyOf<Cfg> = keyof TypesOf<Cfg> | Vue2ComponentKey;

/** 能不能从配置项里解析出组件: 直接给 type(含懒加载), 要么给了非空 actions */
type IsComponentLike<A> = A extends { type: Vue2ComponentKey }
  ? true
...
> extends TypedComponentActionFactories<Cfg> {
  <P extends FieldPathsOf<Root>, K extends ComponentKeyOf<Cfg> | undefined>(
    ...
    actions: readonly CompEntryAction<
      Root,
      P,
      [K] extends [undefined] ? DefaultTables<Root, Cfg, P> : TablesOfComponent<ComponentOf<Cfg, K>>
    >[],
  ): FieldEntry;
...
>(
  ...
  const define = createDefineComponentEntry(ɵtypedFieldActions) as unknown as DefineComponentEntry<
    S,
    UnwrapConfig<C>
  >;
