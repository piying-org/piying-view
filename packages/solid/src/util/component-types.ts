 ...
 * 组件 -> props。
 *
 * Solid 与 React 同构: 函数组件取首参, class 组件取实例的 props 字段, 懒加载先解开一层。
 * `lazy()` 返回的组件本身就是「props 已展开」的函数组件, 走首参那条分支。
 */
...
/** Solid 内置在 props 上的 key, 既不是业务 input 也不是业务 output */
type BuiltinPropKey = 'ref' | 'key' | 'classList';
 ...
 * 值为函数的 prop。
 *
 * Solid 侧 outputs 是以「prop 名原样」下发的(field-template 把 inputs/outputs/attributes
 * 一起 spread 成 props), 所以函数 prop 就是 output, 不做 onXxx -> xxx 的重命名。
 * `any` 形态必须排除, 否则 `value?: any` 会被当成 output。
 */
type FunctionPropKey<T> = {
  [K in Exclude<StringKey<T>, BuiltinPropKey>]: IsAny<T[K]> extends true
    ? never
...
export type SolidComponentKey = SolidComponent | LazyMarkType<any>;
 ...
 */
export type SolidStandardAttrName = Exclude<
  Extract<keyof JSX.HTMLAttributes<unknown>, string>,
  `on${string}`
>;

/** 标准名 ∪ 任意自定义名(data-* 与组件自有的透传属性等) */
export type SolidAttributeName = SolidStandardAttrName | (string & {});
