 ...
 *
 * Solid 的 attributes 最终也是 spread 成 props 落到根元素上,
 * 所以标准名直接取自 Solid 自带的 JSX.HTMLAttributes(摘掉 onXxx), 值仍保持宽松。
 */
...
export type TypedComponentActionFactories<Cfg = unknown> = Omit<
  ...
  'models' | 'attributes'
> & {
  attributes: TypedAttributesActionsFactory<SolidAttributeName>;
};

/** 可用的组件标识: 配置里注册的类型 key, 直接传组件, 或者直接传懒加载函数 */
export type ComponentKeyOf<Cfg> = keyof TypesOfKey<Cfg> | SolidComponentKey;

type TypesOfKey<Cfg> = NonNullable<Cfg extends { types?: infer T } ? T : never>;

/** 能不能从配置项里解析出组件: 直接给 type(含懒加载), 要么给了非空 actions */
type IsComponentLike<A> = A extends { type: SolidComponentKey }
  ? true
