...
// React 适配层直接拿 define.type 渲染, 懒加载必须用 React.lazy 包一层
const reactLazyEmit = lazy(() =>
  import('./component/typed-emit').then(({ TypedEmit }) => ({
    default: TypedEmit,
  })),
);
const lazyDefine = { types: { emit: { type: reactLazyEmit } } };
  ...
  it('类型: 值类型不匹配会被逐个拦下', () => {
    ...
    const badMeta = typedFieldComponentPipe(numOnly, typedDefine, (d) => [
      d(['num'], 'typed', [
        ...
        d.inputs.patch({ meta: { id: 1, name: 2 } }),
      ]),
    ]);
  ...
  it('类型: emit 回调参数写错会被拦下', () => {
    ...
    const badEnd = typedFieldComponentPipe(numOnly, typedDefine, (d) => [
      d(['num'], 'multi', [
        ...
        d.outputs.patch({ onRange: (_start: number, _end: number) => {} }),
      ]),
    ]);
...
describe('typedFieldComponentPipe - outputs 强类型', () => {
  ...
  it('React 版不暴露 models: 没有双向绑定通道', () => {
    const bad = typedFieldComponentPipe(numOnly, baseDefine, (d) => [
      d(['num'], 'emit', [
        // @ts-expect-error React 运行时不消费 field.models
        d.models.patch({}),
      ]),
    ]);
...
describe('typedFieldComponentPipe - attributes / events / wrappers', () => {
  ...
  it('attributes 不再把 onXxx 当成属性名, 但自定义名依旧放行', () => {
    const classOk: 'className' extends ReactStandardAttrName ? true : false =
      true;
    const ariaOk: 'aria-label' extends ReactStandardAttrName ? true : false =
      true;
    const eventExcluded: 'onCopy' | 'onSubmit' extends ReactStandardAttrName
      ? true
      : false = false;
    const customOk: 'my-attr' extends ReactAttributeName ? true : false = true;
...
describe('typedFieldComponentPipe - 运行时', () => {
  ...
  it('每条 entry 都自动下发 setComponent', async () => {
    ...
    expect(
      instance.container.querySelector('.inputs-test')?.innerHTML,
...
describe('typedFieldComponentPipe - 懒加载组件', () => {
  ...
  it('类型 + 运行时: React.lazy 配置下依旧收窄并渲染', async () => {
    const markedDefine = { types: { emit: { type: reactLazyEmit } } };
    ...
    const { instance } = await createComponent(merged, undefined, {
      defaultConfig: { types: { emit: { type: reactLazyEmit } } },
    });
    await delay(80);
