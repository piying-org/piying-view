...
describe('hello', () => {
  ...
  it('group=>string', async () => {
    ...
    await delay();
    expect(el.value).eq('123');
    expect(modelChange$.value).deep.eq({ k1: '123' });
  });
  it('group=>group=>string', async () => {
    ...
    await delay();
    expect(el.value).eq('123');
    expect(modelChange$.value).deep.eq({ o1: { k1: '123' } });
  });
  it('arry=>string', async () => {
    ...
    await delay();
    expect(el.value).eq('123');
    expect(modelChange$.value).deep.eq(['123']);
  });
  ...
  it('boolean', async () => {
    ...
    await delay();
    expect(modelChange$.value).eq(false);
  });
  it('radio', async () => {
    ...
    await delay();
    expect(modelChange$.value).eq('v2');
  });
  it('number', async () => {
    ...
    expect(modelChange$.value).eq(456);
  });
  it('range', async () => {
    const schema = v.pipe(v.number(), setComponent('range'));
    const model = 10;
    const { instance, modelChange$ } = await createComponent(schema, model);
    const el = instance.container.querySelector('input')!;
    expect(el.value).eq('10');
    setInputValue(el, '20');
    await delay();
    expect(el.value).eq('20');
    expect(modelChange$.value).eq(20);
  });
